...
static std::string filter_mode = "blacklist"; // safe default
...
static ADDRINT initialInsAddress = 0;
static std::vector<ADDRINT> insAddresses;

/** Décalage (delta) appliqué à l'entrée de la librairie pour obtenir insAddresses. */
...
static const size_t STRUCT_DUMP_SIZE = 1024;
...
void LoadResultAnalyse(const std::string &filename)
{
    ...
    if (kv.count("Delta"))
    {
        delta = strtoull(kv["Delta"].c_str(), nullptr, 10);
    }
    else
    {
    ...
    if (kv.count("Adresse"))
    {
        initialInsAddress = strtoull(kv["Adresse"].c_str(), nullptr, 16);
        insAddresses.push_back(initialInsAddress);
    }
    else
    {
    ...
    log_out
        ...
        << " delta=" << delta
        << " initialInsAddress=0x" << std::hex << initialInsAddress << std::dec
        << " funName=" << funName
        << " insName=" << insName
        << std::endl;
    file.close();
...
VOID ImageLoad(IMG img, VOID *)
{
    ...
    {
        ADDRINT newInsAddress = IMG_LowAddress(img) + delta;
        insAddresses.push_back(newInsAddress);
        ...
        log_out << "[Backtracer] Nouvelle Adresse cible accumulée: 0x" << std::hex << newInsAddress << std::dec << std::endl;
    }
}
 ...
 */
VOID LeakSecret(const char *regName, ADDRINT ip, ADDRINT value, THREADID tid)
{
    if (value == 0)
        return;

    std::vector<uint8_t> buffer(STRUCT_DUMP_SIZE);
    UINT32 copied = PIN_SafeCopy(buffer.data(), reinterpret_cast<void *>(value), STRUCT_DUMP_SIZE);
    if (copied > 0)
    {
        gLeakedBytes.insert(gLeakedBytes.end(), buffer.begin(), buffer.begin() + copied);
        log_out << "[Backtracer] leak @ ip=0x" << std::hex << ip << " reg=" << regName << " val=0x" << value << " -> Leaked bytes: ";
    ...
    else
    {
        ...
        log_out << "Leaked bytes: ";
        for (size_t i = 0; i < STRUCT_DUMP_SIZE; i++)
        {
            log_out << "00";
            if (i + 1 < STRUCT_DUMP_SIZE) log_out << ' ';
        }
        log_out << std::endl;
    }
}
...
VOID Instruction(INS ins, VOID *)
{
    ...
    for (ADDRINT targetAddr : insAddresses) {
        if (targetAddr == 0) continue;
        if (addr == targetAddr) isTarget = true;
        if (addr >= targetAddr - RANGE_BEFORE && addr <= targetAddr + RANGE_AFTER) inWindow = true;
    }

    // Leak the candidate argument registers at the target entry so we can
    // identify which slot actually carries the secret-bearing structure.
    if (isTarget)

    {
        INS_InsertCall(ins, IPOINT_BEFORE, AFUNPTR(LeakSecret),
                       IARG_PTR, "RCX",
                       IARG_INST_PTR,
